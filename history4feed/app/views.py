...
class FeedView(viewsets.ModelViewSet):
    ...
    @extend_schema(
        ...
        description=textwrap.dedent(
             ...
             * `force_full_fetch` (required, boolean): by default the behaviour (`false`) will check for new posts on this blog since the last post time. In some cases you might want to consider all posts. For example, setting to `false` can miss updates that have happened to currently indexed posts (where the RSS or ATOM feed or search results do not report the updated date correctly -- which is actually very common). To solve this, you can set this setting to `true`. This will then get all URLs available on the blog (same as when adding a new feed), compare these URLs to those for posts indexed, and then fetch posts for URLs not already indexed.
