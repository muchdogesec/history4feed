...
class FeedView(viewsets.ModelViewSet):
    ...
    @extend_schema(
        ...
        description=textwrap.dedent(
            ...
            * `use_search_index` (optional, default is `false`): If the `url` is not a valid RSS or ATOM feed you should set this to `true`, else an error will be returned in the Job created. Set to `true` this mode uses search results that contain the base `url` passed vs. the RSS/ATOM feed entries (when this mode is set to `false`). This mode is only be able to index results in Google Search, so can miss some sites entirely where they are not indexed by Google. Note, you can use the skeleton endpoint to create a feed manually from a non RSS/ATOM URL or where search results do not satisfy your use case.
